// Copied from https://github.com/tomberek/aor-postgrest-client
// Edited to fix https://github.com/tomberek/aor-postgrest-client/issues
...
export default (apiUrl, httpClient = fetchJson) => {
  ...
  const convertRESTRequestToHTTP = (type, resource, params) => {
    ...
    if (parsedCookie.token) {
      options.headers.set('Authorization', `Token ${parsedCookie.token}`)
    }
  ...
  const convertHTTPResponseToREST = (response, type, resource, params) => {
    ...
    switch (type) {
      ...
      case GET_MANY_REFERENCE:
        ...
        const maxInPage =
          parseInt(
            headers.get('content-range').split('/')[0].split('-').pop(),
            10
          ) + 1
        return {
          data: json.map(x => x),
